#!/bin/bash
#
# 事前に作成したパーティション名のリストを元にデータを取得する。特定のパーティションのみ取得したい場合などに使用
#
# 1.SQL作成       -- Partition List は既存のものを使用する。第4パラメータでリストの末尾番号指定可
...
PART_LIST="./csv/${TBL}_part${LIST_NO}.lst"             # リストの場所は/csv直下
...
for csvfile in $( aws s3 ls ${S3PATH}/${DAY_DT}/ | grep ${TBL} | awk '{print $4}' ); do
  psql -h XXXXX.redshift.amazonaws.com -U DBユーザ -d DB名 -p ポート -c \
    "copy ${SCHEMA}.${TBL} from '${S3PATH}/${DAY_DT}/${csvfile}' CREDENTIALS 'aws_access_key_id=XXXXX;aws_secret_access_key=XXXXX' GZIP CSV IGNOREHEADER 1 DELIMITER ',' DATEFORMAT 'auto';" \
